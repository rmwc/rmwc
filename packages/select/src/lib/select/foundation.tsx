...
export const useSelectFoundation = (
  ...
  useEffect(() => {
    ...
    if (value !== undefined && value !== foundationValue) {
      ...
      setSelectedTextContent(getTranslatedValue(props.options, value));
  ...
  useEffect(() => {
    if (defaultValue) {
      const translatedDefaultValue = getTranslatedValue(
        props.options,
        defaultValue
      );
      setSelectedTextContent(translatedDefaultValue.toString());
    }
  }, []);
