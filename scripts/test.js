...
fs.readdir('react-versions', function (err, files) {
  ...
  reactVersions.forEach((reactVersion, index, arr) => {
    ...
    childProcess.execSync(
      //prettier-ignore
      `export REACT_TEST_VERSION=${versionNum} CI=true && react-app-rewired test --transformIgnorePatterns \"node_modules/(?!@material/segmented-button)/\" --env=jsdom ${isFirstRun ? '--coverage' : ''}`,
      { stdio: [0, 1, 2] }
    );
  });
});
